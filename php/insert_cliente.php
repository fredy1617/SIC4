...
if($Anticipo > $CostoTotal){
	...
	if(mysqli_num_rows(mysqli_query($conn, "SELECT * FROM clientes WHERE nombre='$Nombres' AND telefono='$Telefono' AND lugar='$Comunidad' AND direccion='$Direccion' AND referencia='$Referencia'"))>0){
	 		...
	 		$sql = "INSERT INTO clientes (nombre, telefono, lugar, direccion, referencia, total, dejo, paquete, fecha_registro,registro) 
				VALUES('$Nombres', '$Telefono', '$Comunidad', '$Direccion', '$Referencia', '$CostoTotal', '$Anticipo', '$Paquete', '$Fecha_hoy','$Usuario')";
		    ...
		    if ($Anticipo != 0) {	
				...
				$sql = "INSERT INTO pagos (id_cliente, descripcion, cantidad, fecha, tipo, id_user, corte, tipo_cambio) VALUES ($IdCliente, '$Descripcion', '$Anticipo', '$Fecha_hoy', '$Tipo', $id_user, 0, '$Tipo_Campio')";
				if(mysqli_query($conn, $sql)){
